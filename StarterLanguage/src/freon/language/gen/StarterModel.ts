...
import { DummyUnit } from "./internal.js";

import { makeObservable, action, runInAction } from "mobx";

/**
 * Class StarterModel is the implementation of the model with the same name in the language definition file.
 * It uses mobx decorators to enable parts of the language environment, e.g. the editor, to react
 * to any changes in the state of its properties.
 */
export class StarterModel extends MobxModelElementImpl implements FreModel {
    /**
     * A convenience method that creates an instance of this class
     * based on the properties defined in 'data'.
     * @param data partial object
     */
    static create(data: Partial<StarterModel>): StarterModel {
        const result = new StarterModel(data.$id);
        if (!!data.name) {
    ...
    readonly $typename: string = "StarterModel"; // holds the metatype in the form of a string
    ...
    units: DummyUnit[]; // implementation of part 'units'
     ...
     * @param toBeMatched
     */
    public match(toBeMatched: Partial<StarterModel>): boolean {
    ...
    replaceUnit(oldUnit: FreModelUnit, newUnit: FreModelUnit): boolean {
        ...
        if (oldUnit.freLanguageConcept() === "DummyUnit" && oldUnit.freOwnerDescriptor().propertyName === "units") {
            AST.changeNamed("removeUnit", () => {
                const index = this.units.indexOf(oldUnit as DummyUnit);
                this.units.splice(index, 1, newUnit as DummyUnit);
            });
        } else {
            return false;
        }
            ...
            switch (myMetatype) {
                case "DummyUnit": {
                    AST.changeNamed("addUnit", () => {
                        this.units.push(newUnit as DummyUnit);
                    });
                    return true;
                }
            }
    ...
    removeUnit(oldUnit: FreModelUnit): boolean {
        if (!!oldUnit) {
            ...
            switch (myMetatype) {
                case "DummyUnit": {
                    AST.changeNamed("removeUnit", () => {
                        this.units.splice(this.units.indexOf(oldUnit as DummyUnit), 1);
                    });
                    return true;
                }
            }
    ...
    newUnit(typename: string): FreModelUnit {
        switch (typename) {
            case "DummyUnit": {
                let unit: DummyUnit;
                runInAction(() => {
                    unit = DummyUnit.create({});
                });
                AST.changeNamed("newUnit", () => {
                    this.units.push(unit as DummyUnit);
                });
                return unit;
            }
        }
