...
import { DummyUnitSyntaxAnalyserPart, StarterModelCommonSyntaxAnalyserPart } from "./index.js";

/**
 *   Class StarterModelSyntaxAnalyser is the main syntax analyser.
 ...
 *
 */
export class StarterModelSyntaxAnalyser implements SyntaxAnalyser {
    sourceName: string = "";
    locationMap: any;
    private _unit_DummyUnit_analyser: DummyUnitSyntaxAnalyserPart = new DummyUnitSyntaxAnalyserPart(this);
    private _unit_common_analyser: StarterModelCommonSyntaxAnalyserPart = new StarterModelCommonSyntaxAnalyserPart(this);
    ...
    public transformSharedPackedParseTreeNode(node: SPPTNode): any {
        if (!!node) {
            try {
                ...
                if (e.message.startsWith("Syntax error in ") || e.message.startsWith("Error in StarterModelSyntaxAnalyser")) {
                    throw e;
                } else {
    ...
    private transformSharedPackedParseTreeBranch(branch: SPPTBranch): any {
        ...
        if ("DummyUnit" === brName) {
            return this._unit_DummyUnit_analyser.transformDummyUnit(branch);
        } else if ("__fre_reference" === brName) {
            return this.transform__fre_reference(branch);
        } else {
            throw new Error(`Error in StarterModelSyntaxAnalyser: ${brName} not handled for node '${branch?.matchedText}'`);
        }
    }
